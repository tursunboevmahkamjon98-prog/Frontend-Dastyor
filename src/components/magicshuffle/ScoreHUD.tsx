...
/** The always-on gameplay HUD. Deliberately pinned to the very top and
 * bottom edges with nothing in the middle band, so the clown and the three
 * boxes — the actual game — are never covered. */
export default function ScoreHUD({
  ...
  /** Chosen by the player on the setup screen, so not a constant. */
  totalRounds: number;
  score: number;
  lives: number;
  streak: number;
